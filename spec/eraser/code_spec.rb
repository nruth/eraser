...
describe "elements_indexed_by_bitmask(array, bitmask)" do
  ...
  describe "arrays are indexed in reverse order by the mask, since bitmask lsb is on the right, but array reads left-right" do
    let(:array) {[:a, :b, :c, :d]}
    it "pulls out single elements" do
      Eraser::Code.elements_indexed_by_bitmask(array, 0b1000).should == [:d]
      Eraser::Code.elements_indexed_by_bitmask(array, 0b0100).should == [:c]
      Eraser::Code.elements_indexed_by_bitmask(array, 0b0010).should == [:b]
      Eraser::Code.elements_indexed_by_bitmask(array, 0b0001).should == [:a]
    end
    ...
    it "pulls out 2 elements with 1100 0110 etc" do
      ...
      Eraser::Code.elements_indexed_by_bitmask(array, 0b1010).should == [:b, :d]
